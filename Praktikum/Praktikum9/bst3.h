...
/* Mencari node dengan nilai maksimum dalam BST */
void preBeautifulView(BinTree p);
/* I.S. p mungkin kosong
   F.S. Menampilkan tree secara preorder
        Menampilkan x jika pohon kosong */
/* Proses:
   - Jika p = NIL, tampilkan (x)
   - Jika p tidak kosong, tampilkan (ROOT(p).key) dan panggil rekursif ke LEFT(p) dan RIGHT(p) */
/* Notes:
   - Sesuaikan dengan expectation output pada soal */

void inBeautifulView(BinTree p);
/* I.S. p mungkin kosong
   F.S. Menampilkan tree secara inorder
        Menampilkan x jika pohon kosong */
/* Proses:
   - Jika p = NIL, tampilkan (x)
   - Jika p tidak kosong, panggil rekursif ke LEFT(p) dan RIGHT(p) lalu tampilkan (ROOT(p).key) */
/* Notes:
   - Sesuaikan dengan expectation output pada soal */

void postBeautifulView(BinTree p);
/* I.S. p mungkin kosong
